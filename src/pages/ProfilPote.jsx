...
export default function ProfilPote({ poteId, poteContact, onBack }) {
  ...
  const fetchFromDB = async () => {
    const [{ data: p }, { data: g }, { data: s }] = await Promise.all([
      supabase.from('profiles').select('*').eq('id', poteId).single(),
      supabase.from('user_games').select('*').eq('user_id', poteId).order('hours_played', { ascending: false }),
      supabase.from('statuses').select('*').eq('user_id', poteId).gt('expires_at', new Date().toISOString()).single()
    ])
    setProfile(p)
    setGames(g || [])
    setStatus(s)
